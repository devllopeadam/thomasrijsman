 ...
 * Build: http://modernizr.com/download/#-fontface-backgroundsize-borderimage-borderradius-boxshadow-flexbox-hsla-multiplebgs-opacity-rgba-textshadow-cssanimations-csscolumns-generatedcontent-cssgradients-cssreflections-csstransforms-csstransforms3d-csstransitions-applicationcache-canvas-canvastext-draganddrop-hashchange-history-audio-video-localstorage-postmessage-sessionstorage-websockets-webworkers-geolocation-inlinesvg-smil-svg-svgclippaths-touch-webgl-shiv-cssclasses-addtest-prefixed-teststyles-testprop-testallprops-hasevent-prefixes-domprefixes-css_backgroundsizecover-css_boxsizing-css_mask-load
 */
;
...
(function($, undefined) {
  ...
  $.fn.translate = function(x, y) {
    if(Modernizr.csstransforms3d) {
      this[0].style[transform] = 'translate3d(' + x + ',' + y + ',0)';
    } else if(Modernizr.csstransforms) {
      this[0].style[transform] = 'translate(' + x + ',' + y + ')';
    } else {
...
(function(root, undefined) {
  var Cache = root.Cache = function() {
    var self = this,
        items = {};

    self.get = function(name) {
      if(items.hasOwnProperty(name)) return items[name];
      return null;
    };
    ...
    self.set = function(name, content) {
      return items[name] = content;
    };
  };
})(this);
...
(function(window, undefined) {
  ...
  window.App = {
    ...
    register: function(callback) {
      behaviour.push(callback);
    },
    ready: function(callback) {
      ready.push(callback);
    },
    behave: function($context) {
      App.run(behaviour, $context);
    },
    ...
    page: function(ctx) {
      var segments = ctx.params;

      if(App.CONST.modal && !ctx.params.id)
        App.removeModal()

      if(segments.id) {
        App.getModal(ctx.pathname, ctx);
      }

      if(ctx.init && segments.id) App.getPage('/' + ctx.params.page, ctx);
      if(App.CONST.ctx && ctx.path !== App.CONST.path) App.getPage(ctx.path, ctx);

      App.CONST.ctx = ctx;
    },
    getModal: function(url, ctx) {
      App.startLoading();
      if (!ctx.init) {
    ...
    showModal: function(url, ctx, $el) {
      App.doneLoading();

      if(App.CONST.modal) {
        var oldModal = App.CONST.modal.find('.modal-wrapper');
        var newModal = $el.find('.modal-wrapper').css('transform', 'translateX(100%)');
        $('.modal').append(newModal);
        App.CONST.modal.find('.modal-controls').html($el.find('.modal-controls'));

        requestAnimFrame(function() {
          oldModal.velocity('scroll', {
            container: App.CONST.modal,
            complete: function() {
              oldModal.velocity({
                translateX: ['-5%', '0%'],
                translateZ: 0,
        ...
        App.behave(App.CONST.modal);
      } else {
        App.CONST.modal = $el.css('opacity', 0);

        requestAnimFrame(function() {
          if(!ctx.init) $(document.body).append(App.CONST.modal);
          ...
          $el.find('.modal-wrapper').css('opacity', 0).velocity({
            translateY: [0, '10vh'],
            translateZ: 0,
            opacity: [1,0]
          }, {
    ...
    removeModal: function(ctx, next) {
      $(document.body).removeClass('show--modal');
      $('html').removeClass('show--modal');
      App.CONST.modal.find('.modal-wrapper').velocity({
        translateY: ['10vh', 0],
        translateZ: 0,
        opacity: [0,1]
      }, 400);

      App.CONST.modal.velocity({
        opacity: [0,1]
      }, {
        display: 'none',
        duration: 200,
      ...
      App.CONST.modal = null;

      if (next) next();
    },
    getPage: function(url, ctx) {
      App.startLoading();
      if(!ctx.init || ctx.init && ctx.params.id) {
        $.get(url, function(result) {
          App.title(result);
          var $el = $('<div />').html(result);
          $el = $el.find('.contents');
          App.renderPage(url, ctx, $el);
        });
      }
    },
    renderPage: function(url, ctx, $el) {
      if($('.contents').length) {
        $('.contents').html($el.html());
        $(window).scrollTop(0);
      } else {
      ...
      $('.contents').removeClass(function (index, css) {
        return (css.match (/\page--\S+/g) || []).join(' ');
      }).addClass($el.attr('class'));
      App.behave($(document.body));
    },
...
App.register(function($context) {
  $context.find('.projects-filter-submit').text(function() {
    var blocks = $('.main .block--project').length;
    return blocks + ' resultaten';
  })

  $context.find('.projects-filter-open').off().on('click', function() {
    $('.main').velocity('scroll', {
      offset: 0 - $('.nav--sticky').innerHeight()
    });
  });

  $context.find('form.projects-filter').off('change submit');
  $context.find('form.projects-filter').on('change', function(e) {

    var serialize = $(this).serialize()
    var url = window.location.origin + window.location.pathname + '?' + serialize;

    $.get(url, function(result){
      var $el = $('<div />');
      $el.html(result);

      var blocks = $el.find('.main .block--project');

      $('.projects-filter-submit').text(blocks.length + ' resultaten');

      $('.main .block--project').remove();
      $('.fn-masonry').append(blocks);

      $('.main').imagesLoaded(function() {
        $('.fn-masonry').each(function() {
      ...
      App.behave(blocks);
      $context.find('.projects-filter-open').click();
    })
...
$(function() {
  return $(document.body).on('click', 'a', function(e) {
    ...
    if (domain(link) === domain(window.location.href) || domain(link) === 'stadco.dev') {
      if (e) {
        e.preventDefault();
      }
...
App.register(function($context) {
  $context.find('.live-filter input').off('change');
  $context.find('.live-filter input').on('change', function(e) {
    e.preventDefault();

    console.log($(this).val())
    console.log($(this).attr('name'))

    $('main .block[data-type='+$(this).attr('name')+']')

    // $('main .block').css('display', 'none');
    //
    // if ($(this).val() == 'on') {
    //   $('main .block[data-type='+$(this).attr('name')+']').css('display', 'block');
    // }
    ...
    //   var $el = $('<div />');
    //   $el.html(result);
    //   $blocks = $el.find('.block:not(.block--live-filter, .block--newsletter)');
    //
    //   $('.main .block:not(.block--live-filter, .block--newsletter)').remove();
    //   $('.main').append($blocks);
    //
    //   App.behave($blocks);
    //
    //   $('.fn-masonry').imagesLoaded(function() {
    //
    //     $('.fn-masonry').each(function() {
    //       if($(this).data('isotope')) {
    //         $(this).isotope('destroy');
  ...
  $context.find('.live-filter-button').on('click', function() {
    $('.main').velocity('scroll', {
      offset: 0 - $('.nav--sticky').innerHeight()
    });
  });
...
// TODO: move to bricklayer oid http://ademilter.github.io/bricklayer/

App.register(function($context) {
  $context.find('.fn-masonry').each(function() {
...
App.register(function($context) {
  if($(window).width() < 568) {
    $context.find('.nav--main').on('click', function() {
...
App.register(function($context) {
  $context.find('.fn-close-modal').on('click', function(e) {
    if(!history.emulate) {
      e.preventDefault()
      page('/' + App.CONST.ctx.params.page);
    }
  });
  ...
  $(window).keyup(function(e) {
    if(App.CONST.modal) {
      ...
      if (e.keyCode == 39 && App.CONST.modal.find('.modal-control--next')) {
        App.CONST.modal.find('.modal-control--next').click()
      }
      if (e.keyCode == 37 && App.CONST.modal.find('.modal-control--prev')) {
        App.CONST.modal.find('.modal-control--prev').click()
      }
    }
  })
});
...
App.register(function($context) {
  if($context.find('.nav--main').length < 2) {
    var $nav = $context.find('.nav--main').clone();
    $nav.insertAfter($('.nav--main')).addClass('nav--sticky nav--sticky__invisible');
  }
});

App.ready(function() {
  $(window).on('scroll', _.throttle(function(){
    if($(this).scrollTop() > 0 && $('.nav--sticky').hasClass('nav--sticky__invisible')) {
      $('.nav--sticky').removeClass('nav--sticky__invisible');
    } else if($(this).scrollTop() <= 0 && !$('.nav--sticky').hasClass('nav--sticky__invisible')) {
      $('.nav--sticky').addClass('nav--sticky__invisible');
    }
  }, 10))
})
...
App.register(function($context) {
  if (Modernizr.touch) {
    $context.find('.block--team').on('click', function(e) {
      if(!$(this).hasClass('active')) {
        e.preventDefault();

        $(this).addClass('active')
        $(document).one('touchmove', function() {
          $context.find('.block--team').removeClass('active');
        })
