...
;
  ...
  (function() {
    ...
    return $.fn.translate = function(x, y) {
      if (Modernizr.csstransforms3d) {
        return this[0].style[transform] = 'translate3d(' + x + ',' + y + ',0)';
      } else if (Modernizr.csstransforms) {
        return this[0].style[transform] = 'translate(' + x + ',' + y + ')';
      } else {
        return this.css({
          top: x,
          left: y
  ...
  (function(root) {
    var Cache;
    return Cache = root.Cache = function() {
      var items, self;
      self = this;
      items = [];
      ...
      return this.set = function(name, content) {
        items[name] = content;
  ...
  (function(window) {
    ...
    $body = window.App = {
      ...
      register: function(callback) {
        return behaviour.push(callback);
      },
      ready: function(callback) {
        return ready.push(callback);
      },
      behave: function($context) {
        return App.run(behaviour, $context);
      },
      run: function(collection, $context) {
        var i, results;
        i = 0;
        results = [];
      ...
      boot: function() {
        ...
        App.run(ready, $body);
        App.behave($body);
        page('/:page/:id', App.project);
        page('/over', App.info);
        page.exit('/over', App.exitInfo);
      ...
      home: function(ctx) {
        ...
        App.filter();
        $('.nav--projects').removeClass('nav--activated').find('li').removeClass('nav-active');
        $('.nav--projects').removeClass('nav--inside').find('li').removeClass('nav-inside');
        if (App.PROJECT) {
          App.hideProject(false);
        } else if (!ctx.init) {
          scrollTop = $(window).scrollTop();
          offset = $('.projects').offset().top;
          if (scrollTop > offset) {
            $('.projects').velocity('scroll', App.ANIMATION_DURATION);
          } else if (scrollTop <= offset + 1 && scrollTop !== 0) {
            $(document.body).velocity('scroll', App.ANIMATION_DURATION);
          }
        }
        App.PREV = App.CTX;
        return App.CTX = ctx;
      ...
      info: function(ctx) {
        if (App.CTX && App.CTX.path === ctx.path) {
          return false;
        }
        $(document.body).css('overflow', 'hidden');
        $('main .nav--main').each(function() {
          var offset;
          offset = $(window).scrollTop() - $(this).parents('main').offset().top;
          return $(this).css({
            'position': 'absolute',
            'top': offset
          });
        ...
        _.defer(function() {
          $('.header--intro, main, .biography').velocity({
            translateX: ['-100%', '0%'],
            translateZ: 0
          });
          $('main .nav--main').velocity({
            translateX: ['100%', '0%'],
            translateZ: 0
          });
          return $('.biography nav').velocity({
            translateX: ['0%', '-100%'],
            translateZ: 0
          });
        });
      ...
      exitInfo: function(ctx, next) {
        ...
        _.defer(function() {
          ...
          $('main .nav--main').velocity({
            ...
            complete: function() {
              return $('main nav, main').removeAttr('style');
            }
          });
          return $('.biography nav').velocity({
            translateX: ['-100%', '0%'],
      ...
      project: function(ctx) {
        if (App.PROJECT) {
          App.hideProject(true);
        }
        if (App.CTX && ctx.path !== App.CTX.path) {
          App.fetch(ctx.path, ctx, App.showProject);
        }
        if (ctx.init) {
          App.PROJECT = $('.project');
      ...
      showProject: function(ctx, body) {
        var $project;
        App.PROJECT = $project = $(body).filter('.project').css('display', 'none');
        $(document.body).find('.header--intro').after($project);
        $(document.body).css('overflow', 'hidden');
        if ($(window).scrollTop() < $('.projects').offset().top) {
          $('.projects').velocity('scroll', App.ANIMATION_DURATION / 2);
        }
        $('.nav--projects').removeClass('nav--activated').find('li').removeClass('nav-active');
        $('.nav--projects').removeClass('nav--inside').find('li').removeClass('nav-inside');
        $('.nav--projects').addClass('nav--inside').find('li[data-filter="' + $project.data('category') + '"]').addClass('nav-inside');
        $('.projects .grid').velocity({
          opacity: [0, 1],
          translateZ: 0
        }, {
          duration: App.ANIMATION_DURATION
        });
        $project.find('.grid').velocity({
          translateY: ['0vh', '17.5vh']
        }, {
          duration: App.ANIMATION_DURATION * 2
        });
        return $project.velocity({
          ...
          duration: App.ANIMATION_DURATION,
          delay: App.ANIMATION_DURATION,
          display: 'block',
          complete: function() {
            return App.behave($project);
          }
        });
      },
      hideProject: function(transition) {
        var $project, delay;
        $project = App.PROJECT;
        delay = transition ? App.ANIMATION_DURATION * 1.5 : 0;
        $(document.body).css('overflow', 'initial');
        $('.nav--projects').removeClass('nav--inside').find('li').removeClass('nav-inside');
        if (!transition) {
          $('.projects .grid').css('opacity', 0).velocity({
            opacity: 1,
            translateZ: 0
          }, {
            duration: App.ANIMATION_DURATION,
      ...
      filter: function(ctx) {
        ...
        if (filter) {
          ...
          $('.nav--projects li').removeClass('nav-active');
          $('.nav--projects').addClass('nav--activated').find('li[data-filter="' + fltr + '"]').addClass('nav-active');
        } else {
          $('.fn-masonry').isotope({
            filter: '*'
          });
          $('.nav--projects').removeClass('nav--activated').find('li[data-filter="' + fltr + '"]').removeClass('nav-active');
        }
        App.PREV = App.CTX;
        return App.CTX = ctx;
      },
      fetch: function(url, ctx, callback) {
        App.startLoading();
      ...
      render: function(body, ctx, callback) {
  ...
  App.register(function($context) {
    return $context.find('[data-bg]').each(function() {
      var img;
      img = new Image();
      img.src = $(this).data('bg');
      return img.onload = (function(_this) {
        return function() {
          return $(_this).css('background-image', 'url(' + $(_this).data('bg') + ')').removeAttr('data-bg');
        };
      })(this);
    });
  });
  ...
  App.register(function($context) {
    return $context.find('.project-picture-nav').each(function() {
      $('.project-picture-inside').children().addClass('project-picture--hidden').first().removeClass('project-picture--hidden');
      $(this).find('.project-picture-nav-item:first').addClass('project-picture-nav-item--active');
      return $(this).find('.project-picture-nav-item').on('click', function(e) {
        e.preventDefault();
        $('iframe').each(function() {
          this.contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}', '*');
          return this.contentWindow.postMessage('{"method":"pause","value":""}', '*');
        });
        $('.project-picture-nav-item').removeClass('project-picture-nav-item--active');
        $(this).addClass('project-picture-nav-item--active');
        $('.project-picture-inside').children().addClass('project-picture--hidden');
        return $('.project-picture-inside').children().eq($(this).index()).removeClass('project-picture--hidden');
      });
    });
  });

  App.register(function($context) {
    return $context.find('.nav-right a').on('click', function(e) {
      e.preventDefault();
      if (App.CTX && App.CTX.path !== '/over') {
        return App.navigate('/over');
      } else {
        page.replace('/', null, null, false);
        App.exitInfo(null, function() {});
        return App.CTX.path = '/';
      }
    });
  });
  ...
  App.register(function($context) {
    var duration, startType, started;
    $('.header').each(function() {
      var h;
      h = $(this).outerHeight();
      $(this).find('.header-type').css('font-size', Math.round(h * .95));
      return $(this).find('video').each(function() {
        return enableInlineVideo(this);
      });
    });
    started = false;
    duration = 16000;
    startType = function() {
      var width;
      started = true;
      width = $(window).innerWidth() / $context.find('.header-type').innerWidth();
      width = (0 - ((width * 100) + 100)) + '%';
      return $('.header-type').velocity({
        translateX: [width, '0%'],
        translateZ: 0
      }, {
        mobileHA: true,
    ...
    $('.header video').on('play', function(e) {
      duration = Math.floor(this.duration * 1000);
      return _.defer(startType);
    });
    $('.header video').on('loadeddata', function(e) {
      if (!started) {
        startType();
      }
    ...
    return $('.header video').on('stalled error', function(e) {
      return this.load();
    });
  });
  ...
  App.ready(function() {
    return $(window).on('resize orientationchange', function() {
      return $('.header').each(function() {
        var h;
        h = $(this).outerHeight();
        return $(this).find('.header-type').css('font-size', Math.round(h * .95));
      });
    });
  });

  App.register(function($context) {
    return $context.find('.fn-masonry').isotope({
      layoutMode: 'packery',
  ...
  $(function() {
    return $(document.body).on('click', 'a', function(e) {
      ...
      if (domain(link) === domain(window.location.href) || domain(link) === 'thomas-rijsman.dev') {
        if (e) {
          e.preventDefault();
        }
        return App.navigate(link.split(domain(link))[1]);
      } else {
  ...
  App.register(function($context) {
    return $('.no-touch .projects-project:has(video)').on('mouseenter touchstart', function() {
      return $('video', this)[0].play();
    }).on('mouseleave touchend', function() {
      return $('video', this)[0].pause();
    });
  });

  App.register(function($context) {
    $context.find('.header--intro').on('click', function(e) {
      return $context.find('.projects').velocity('scroll', App.ANIMATION_DURATION);
    });
    return $context.find('.projects-header').on('click', function(e) {
